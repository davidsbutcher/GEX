make_every_XIC <- 
   function(
      ...
      target_col_name <- enquo(target_col_name)
      target_sequence_col_name <- enquo(target_sequence_col_name)
      ...
      for (i in seq_along(chemform_withH)) {
         ...
         if (
            stringr::str_detect(chemform_withH[[i]], fixed("S0"))
         ) {
      ...
      sumXIC1 <- 
         ...
         purrr::map(reduce, full_join, .progress = TRUE)
